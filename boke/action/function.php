 ...
 * @LastEditTime: 2023-11-06 11:28:19
...
//待处理，需要删除，用于多域名给授权验证测试
add_filter('zib_admin_tool_action_key', function ($val, $args) {
    $home_url   = ZibPayIncome::replace_url(home_url());
    $key        = substr(md5($home_url), 0, 15);
    $get_option = get_option('home_' . $key);

    $wp_rewrite = array(
        'time'       => zib_microtime_float(),
        'home_url'   => $home_url,
        'key'        => $key,
        'get_option' => $get_option,
        'md5'        => md5($args[2]),
    );
    // file_put_contents(__DIR__ . '/error.json', json_encode($wp_rewrite), FILE_APPEND);
...
function zib_ajax_frontend_links_submit()
{
    ...
    if (empty($_POST['link_yes'])) {
        echo (json_encode(array('error' => 1, 'ys' => 'danger', 'msg' => '请使用一键获取检测友链')));
        exit();
    }
...
function zib_ajax_debounce($key, $args, $time = 3, $msg = '操作过于频繁，请%time%秒后再试')
{
    ...
    if ($db && is_array($db)) {
        foreach ($db as $k => $v) {
            if (strtotime($new_time) > strtotime("+$max_time second", strtotime($v))) {
                unset($db[$k]);
            }
        }
    } else {
    ...
    $is_error = ($old_time && strtotime($new_time) <= strtotime("+$time second", strtotime($db[$key])));
