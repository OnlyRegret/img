 ...
 * @LastEditTime: 2023-11-03 18:50:46
...
function zib_posts_mian_list_list($args = array())
{
    ...
    if ($style == 'style2') {
        ...
        $excerpt = '<div class="item-excerpt muted-color text-ellipsis mb6" style="padding: 10px 15px; border-radius: 8px; background: rgba(141, 141, 141, 0.05); line-height: 1.7;">' . $excerpt . '</div>';
        $class .= ' flex';
...
function zib_get_posts_list_badge($args = array(), $class = 'item-heading text-ellipsis')
{
    ...
    if (!$args['is_tag'] && in_array('tag', $show_badge)) {
        $badeg .= zib_get_posts_tags('but', '<svg t="1701911976616" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="36942" width="20" height="20"><path d="M674.22208 443.72992C569.93792 329.96352 569.93792 193.4336 569.93792 0 245.49376 125.15328 326.6048 466.49344 315.01312 580.27008 233.91232 512 210.7392 352.70656 210.7392 352.70656 118.02624 398.2336 71.68 512 71.68 603.0336 71.68 830.5664 268.66688 1024 512 1024s440.32-182.04672 440.32-420.97664c0-136.52992-104.28416-204.8-104.28416-386.84672-150.6304 45.5168-173.81376 159.29344-173.81376 227.55328z" fill="#FF5050" p-id="36943"></path><path d="M455.07584 583.68c137.04192 53.69856 105.41056 204.05248 105.41056 247.00928 31.63136-32.21504 42.16832-96.65536 42.16832-96.65536C634.27584 755.5072 655.36 798.47424 655.36 841.43104 655.36 938.0864 571.0336 1024 476.16 1024S296.96 938.0864 296.96 841.4208C307.5072 744.78592 455.07584 669.5936 455.07584 583.68z" fill="#FDB350" p-id="36944"></path></svg> ', '', 3);
    }
    ;
...
function zib_posts_mini_while($args = array(), $number = 0)
{
    ...
    if ($args['show_meta']) {
        if (_pz('list_orderby') == 'modified') {
            $time_ago = zib_get_time_ago(get_the_modified_time('Y-m-d H:i:s'));
        } else {
