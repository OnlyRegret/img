 ...
 * @LastEditTime: 2023-09-23 23:08:58
...
function zib_bbs_get_respond()
{
    ...
    $btns_r = '<label class="pw-1em simi" data-placement="top" data-toggle="tooltip" title="你的评论仅评论双方可见。" style="margin-bottom: 0px;background: #ff6ea259;border-radius: 6px;padding: .3em 1em; color: #fc6976;">
...
function zib_bbs_get_comment_title()
{
    ...
    $order = '<a class="but comment-orderby' . (!$corderby || 'comment_date_gmt' == $corderby ? ' b-theme' : '') . '" href="' . esc_url(add_query_arg('corderby', 'comment_date_gmt', $url)) . '">最新</a>';
    $order .= '<a class="but comment-orderby' . ('comment_like' == $corderby ? ' b-theme' : '') . '" href="' . esc_url(add_query_arg('corderby', 'comment_like', $url)) . '">最热</a>';
    ...
    $only_author      = '<a class="but comment-orderby btn-only-author p2-10 em09 ml10" href="' . esc_url(add_query_arg('only_author', ($only_author ? false : $author_id), $url)) . '">' . $only_author_text . '</a>';
