 ...
 * @LastEditTime: 2023-09-17 12:44:35
    ...
    switch ($allow_view) {
        ...
        case 'comment':
            ...
            if (!$user_id) {
                ...
                $con = '<div class="text-center em09 mt20" reply-show="true" reload-hash="#article"><p class="separator muted-3-color mb20">评论后继续查看</p><p><a href="' . $comment_href . '" class="but jb-blue padding-lg">' . zib_get_svg('comment') . '去评论</a></p></div>';
            } elseif (!$data['allow_reason']) {
...
function zib_bbs_get_posts_icon_metas($posts_id = 0)
{
    ...
    $html .= '<item>' . zib_get_svg('view') . _cut_count($views) . zib_get_comment_paginate(_pz('comment_paginate_type'), _pz('comment_paging_ajax_ias_s'), _pz('comment_paging_ajax_ias_max', 3)).'</item>';
