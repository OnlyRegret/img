...
if ($avatar_img == true) {
 ...
 $avatar_img = get_avatarbox_user_toshi($author_id,$cache);
    ...
    $info_html .= '<div class="flex0 header-avatar"><span class="avatar-img " >'.$avatar_img. $avatar . '<div class="user-identity" style="text-align: center;"><span class="but c-red" data-clipboard-tag="ID" data-clipboard-text="'.$author_id.'"><svg class="icon" aria-hidden="true"><use xlink:href="#icon-user-color"></use></svg>UID:'.$author_id.'</span></div></span></div>';
    $info_html .= '<div class="flex1">';
    $info_html .= $name ;
    $info_html .= $identity ? '<div class="user-identity flex ac hh"><span class="but" style="--this-bg: linear-gradient(43deg, #ff6ac34a 0%, #82e1ff 46%, #ff6ac34a 100%);--this-color: #ff5473;"><img src="'.vxras_pz('static_vxras').'/img/toast1.png" style="max-width: 30px;">加入小窝'.zib_get_user_join_days($author_id).'天啦～</span>' . $identity . '</div>' : '';
    $info_html .= $desc;
    $info_html .= '</div>';
    $info_html .= $btns;
    $info_html .= '</div>';
