 ...
 * @LastEditTime: 2023-11-10 21:40:47
...
function zib_ajax_set_user_ban()
{
    ...
    zib_send_json_success(array('msg' => $send_msg, 'hide_modal' => true));
}
...
//签到任务从这里开始
//发起领取奖励
function zib_ajax_checkin_task()
{
    $user_id = get_current_user_id();
    if (!$user_id) {
        zib_send_json_error('请先登录');
    ...
    if (_pz('checkin_reward_task')=='关闭') {
        zib_send_json_error('任务奖励未开启');
    }
    
    global $wpdb;
    $userid = !empty($_REQUEST['userid']) ? (int) $_REQUEST['userid'] : 0;
    //奖励类型
    $task_points   = (string) _pz('checkin_reward_task', 0);
    
    //获奖通知
    $reward_notice_in   = (int) _pz('reward_notice_in', 0);
    ...
    //获取累计签到
    $checkin_all_day = (int) get_user_meta($userid, 'checkin_all_day', true);
    
    //获取数据库
    $wpdb->card = $wpdb->prefix . 'zibpay_card_password';
    $wpdb->xyprotask = $wpdb->prefix . 'xypro_task';
    
    $randpost=$wpdb->get_results("SELECT * FROM $wpdb->card WHERE other='$task_b' and status='0'");
    $xytask = array_rand($randpost,1);
    $card = $randpost[$xytask]->card;
    $pass = $randpost[$xytask]->password;
    $time = current_time("Y-m-d H:i:s");
    
    if(empty($randpost)){
        zib_send_json_error('管理员未设置奖品');
    }
    
    $cxtask=$wpdb->get_results("SELECT card,status FROM $wpdb->xyprotask WHERE user_id='$userid'");
    $user_status = $cxtask[0]->status;
    $user_card = $cxtask[0]->card;
    
    //卡密查重
    if($card == $user_card){
        $xytask = array_rand($randpost,1);
        $randtask=$wpdb->get_results("SELECT card,password FROM $wpdb->card WHERE id='$xytask'");
        $card = $randtask[0]->card;
        $pass = $randtask[0]->password;
    }

    if($task_off=='否'&&!empty($task_s)){ $t='0';$task_s = $task_s+$t;}else{ $t='1'; $task_s = $task_s*($user_status+$t);}
    ...
    $reward_title = '恭喜您！在签到任务中获得奖励';
    $reward_message = '您好！' . $user_name . '<br>';
    $reward_message .= '您在【' . $blog_name . '】有一条奖励提醒<br>';
    $reward_message .= '奖励类型：' . $task_points . '<br>';
    if($task_points == '卡密'){
        $reward_message .= '卡密卡号：' . $card . '<br>';
        $reward_message .= '卡密密码：' . $pass . '<br>';
    }
    $reward_message .= '领取时间：' . $time . '<br>';
    $reward_message .= '<br>';
    $reward_message .= '您可以再次点击签到查看详情';
    
    if($checkin_all_day>=$task_s){
        if($task_off=='否'&&$user_status!='0'){
            $task_message = '未开启后续任务，请联系管理员添加';
        }else{
            $task_array = array(  
                'user_id'=> $userid,  
                'card' => $card,
            ...
            if($user_status == ''){
                $wpdb->insert($wpdb->xyprotask,$task_array);
                $task_message = '成功领取'.$task_s.'天签到奖励';
                if (_pz('reward_notice_in', true)) {
                    ZibMsg::add($msg_arge);
                }
            }else{
                $wpdb->update($wpdb->xyprotask,$task_update_array,array('user_id'=> $userid)); 
                $task_message = '成功领取'.$task_s.'天签到奖励';
                //创建新消息
                if (_pz('reward_notice_in', true)) {
                    ZibMsg::add($msg_arge);
        ...
        $task_message = '请查询是否达标';
    }
    if(empty($task_message)){
        zib_send_json_error('领取错误');
    }
    zib_send_json_success($task_message);
}
add_action('wp_ajax_checkin_task', 'zib_ajax_checkin_task');

//领取记录模态框
function zib_ajax_checkin_record_modal()
{
    $user_id = get_current_user_id();
    if (!$user_id) {
        zib_ajax_notice_modal('danger', '请先登录');
    ...
    $html = zib_get_user_checkin_record_modal($user_id);

    echo $html;
    exit;
}
add_action('wp_ajax_checkin_record_modal', 'zib_ajax_checkin_record_modal');
//签到任务这里结束