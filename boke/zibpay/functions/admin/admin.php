 ...
 * @LastEditTime: 2023-10-20 22:36:28
...
function zib_admin_post_posts_custom_column($column_name, $posts_id)
{
    ...
    switch ($column_name) {
        case "pay_data":
            ...
            $con = '<a style="color: ' . ['#ff4747', '#ee5307', '#1e8608', '#1a8a65', '#0c9cc8', '#086ae8', '#3353fd', '#4641e8', '#853bf2', '#e94df7', '#ca2b7d', '#d7354c', '#ff4747', '#8e24ac'][$zibpay_type] . ';" href="' . add_query_arg(array('zibpay_type' => $zibpay_type)) . '">' . zibpay_get_pay_type_name($zibpay_type) . '</a>';
...
function zib_output_users_columns($var, $column_name, $user_id)
{
    ...
    switch ($column_name) {
        ...
        case "assets":
            ...
            $html .= '<div class="row-actions">' . zibpay_get_user_assets_details_admin_link($user_id, "", "查看明细") . '</div>';
...
function zibpay_bulk_edit_custom_box($column_name, $post_type)
{
    ...
    echo zib_get_quick_edit_custom_input($fields_args, '', '<div class="c-yellow">批量设置付费参数，请仔细检查配置内容，更新后无法撤回。请注意配置逻辑，例如会员价不能高于普通价，执行价不能高于原价等</div>');
}
...
{
    $permissible_posts_type = ['post', ''];
    if (!$update || !in_array($post->post_type, $permissible_posts_type) || empty($_REQUEST['zib_bulk_edit']) || empty($_REQUEST['screen']) || $_REQUEST['screen'] !== 'edit-post') {
        return;
    }

    $zibpay_bulk_edit = $_REQUEST['zib_bulk_edit'];
