 ...
 * @LastEditTime: 2023-11-01 19:13:33
    ...
    switch ($order_type) {
        ...
        case 99: //购买插件授权
            if (!$user_id) {
                ...
                $__data['order_price'] = aut('pay')['pay'];//价格
                $__data['product_id']  = aut('user')['title'];//title名称
                $__data['order_type'] = '7';//购买类型
                // '1' => '付费阅读',
                // '2' => '付费资源',
                // '3' => '产品购买',
                // '4' => '购买会员',
...
function zibpay_zibpay_initiate_paypal($order_data)
{
    ...
    $config['cancel'] = add_query_arg(['return_url' => $return_url, 'cancel' => 'cancel'], ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/shop/paypal/return.php');
    $config['return'] = add_query_arg(['return_url' => $return_url, 'order_num' => $order_data['order_num']], ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/shop/paypal/return.php');
...
function zibpay_initiate_official_alipay($order_data = array(), $payment = 'alipay')
{
    ...
    if (wp_is_mobile() && $config['h5'] && $config['webappid'] && $config['webprivatekey']) {
        ...
        $request->return_url                   = add_query_arg(['return_url' => $request->return_url, 'app_type' => 'wap'], ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/shop/alipay/return.php'); // 新版回调地址测试
...
function zibpay_initiate_official_wechat($order_data = array())
{
    ...
    if (wp_is_mobile() && $config['h5'] && !$zibpay_is_wechat_app) {
        ...
        if ($pay->checkResult()) {
            ...
            //待处理，需要测试
            $redirect_url = add_query_arg(['order_num' => $order_data['order_num'], 'sign' => md5($order_data['order_num'] . 'zib_official_wechat'), 'return_url' => $return_url], ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/shop/weixin/return.php');
        ...
        if ($pay->checkResult()) {
            ...
            $result['jsapi_return'] = add_query_arg(['order_num' => $order_data['order_num'], 'sign' => md5($order_data['order_num'] . 'zib_official_wechat'), 'return_url' => !empty($order_data['return_url']) ? $order_data['return_url'] : home_url()], ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/shop/weixin/return.php');
            return $result;
        } else {
            return array('error' => 1, 'msg' => $pay->getError() . ' ' . $pay->getErrorCode());
        }
...
function zibpay_initiate_xhpay($order_data)
{
    ...
    if ($is_mobile && $payment_method === 'wechat' && (!isset($config['wx_h5']) || $config['wx_h5'])) {
        //手机端微信H5支付
        $data["wap_url"]  = add_query_arg(['order_num' => $order_data['order_num'], 'sign' => md5($order_data['order_num'] . 'zib_xhpay'), 'return_url' => $return_url], ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/shop/xhpay/return.php');
    ...
    if ($is_mobile && $payment_method === 'alipay') {
        ...
        $data["redirect_url"] = add_query_arg(['order_num' => $order_data['order_num'], 'sign' => md5($order_data['order_num'] . 'zib_xhpay'), 'return_url' => $return_url], ZIB_TEMPLATE_DIRECTORY_URI . '/zibpay/shop/xhpay/return.php');
